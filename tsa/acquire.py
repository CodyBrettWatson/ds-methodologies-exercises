...
def merge_dataframes():
    ...
    items_df = pd.read_csv('items.csv')
    stores_df = pd.read_csv('stores.csv')
    sales_df = sales_df.rename(index=str, columns={'store': 'store_id', 'item':'item_id'})
    df_store_sales = sales_df.merge(stores_df, left_on='store_id', right_on='store_id')
    df_final = df_store_sales.merge(items_df, left_on='item_id', right_on='item_id')
